...
import { ClaudeAnalysisResult, ReportOptions, ProjectSummary, ClaudeProject, ClaudeMessage } from '../types';
import { loadTodos } from './analyzer';

export async function generateReport(analysisResult: ClaudeAnalysisResult, options: ReportOptions): Promise<string> {
...
async function generateReportContent(analysisResult: ClaudeAnalysisResult, options: ReportOptions): Promise<string> {
...
async function generateProjectSummary(project: ClaudeProject, targetDate: string): Promise<ProjectSummary> {
...
function generateStats(projectSummaries: ProjectSummary[]): string {
  ...
  const mostActiveProject = projectSummaries.reduce((max, p) => 
    p.messagesCount > max.messagesCount ? p : max, 
